"""
Configuration sampler for Minegym scenario parameters.

Reads a plain-text config file of key/value pairs -- fixed values,
distributions (Normal/Poisson/Binomial/Uniform), or list literals -- and
exposes them through ConfigSampler. Parameters are automatically classified
as "fixed" (sampled once per run) or "episodic" (resampled every episode);
see ConfigSampler._get_fixed_parameter_keys / _get_episodic_parameter_keys
...
class ConfigSampler:
    ...
    def _get_fixed_parameter_keys(self) -> Set[str]:
        ...
        return {
            ...
            # Equipment characteristics / mine layout
    ...
    def _classify_parameter(self, key: str) -> str:
        ...
        return 'episodic'  # unknown parameters default to episodic (safer)
        ...
        for key, value in all_configurations.items():
            ...
            if param_type == 'fixed':
                ...
                if isinstance(value, Distribution):
                    ...
                    # Non-distribution episodic params are treated as fixed.
                    self.fixed_configurations[key] = value
    ...
    def new_episode(self, episode_seed: Optional[int] = None):
        ...
        episode_seed: optional seed for this episode; if None, uses
        seed_value + episode_count (reproducible but varied per episode).
        """
    ...
    def get_sampled_value(self, key: str) -> Union[float, int, List[int]]:
        """
        Get sampled value: scenario overrides, then fixed values, then
        episodic values from the current episode (in priority order).
        """
        ...
        # Auto-initialize an episode if none has been sampled yet.
    ...
    def enable_validation(self):
        ...
        """
        VALIDATION_BOUNDS = {
            # Zone/dump/loading times (minutes)
            ...
            # MTTR / MTBF -- should not exceed shift duration
        ...
        for load_key in ['load_per_trip', 'LO']:  # both possible key names
            if load_key in self.fixed_sampled_data:
                load = self.fixed_sampled_data[load_key]
                if load > 98.2:
                    self.fixed_sampled_data[load_key] = 93.3
                    print(f"WARNING: Reduced {load_key} from {load}t to 93.3t (95% of CAT 777 capacity)")
                break
